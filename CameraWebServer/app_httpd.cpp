...
static void led_blink_task(void *arg) {
    while (led_blinking) {
        enable_led(true);  // Bật đèn LED
        vTaskDelay(1000 / portTICK_PERIOD_MS);  // Delay 1 giây
        enable_led(false); // Tắt đèn LED
        vTaskDelay(1000 / portTICK_PERIOD_MS);  // Delay 1 giây
    }
...
// Function to turn on the LED flash
static esp_err_t led_on_handler(httpd_req_t *req) {
    if (!led_blinking) { // Kiểm tra nếu chưa bật chế độ nhấp nháy
        led_blinking = true;
        xTaskCreate(led_blink_task, "LED Blink Task", 2048, NULL, 1, NULL);
    }
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    return httpd_resp_send(req, "LED Flash Started", HTTPD_RESP_USE_STRLEN);
}


// Function to turn off the LED flash
static esp_err_t led_off_handler(httpd_req_t *req) {
    led_blinking = false;  // Dừng nhấp nháy
    enable_led(false);     // Đảm bảo đèn LED tắt
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    return httpd_resp_send(req, "LED Flash Turned Off", HTTPD_RESP_USE_STRLEN);
}
